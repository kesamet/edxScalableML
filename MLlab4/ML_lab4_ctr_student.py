...
aSparse = SparseVector(len(aDense), xrange(len(aDense)), aDense)

bDense = np.array([0., 0., 0., 1.])
bSparse = SparseVector(len(bDense), xrange(len(aDense)), bDense)
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    idx = []
    for x in rawFeats:
        idx.append(OHEDict[x])
    return SparseVector(numOHEFeats, sorted(idx), [1.0]*len(idx))
...
def evaluateResults(model, data):
    ...
    """
    predictions = data.map(lambda x: (x.label, getP(x.features, model.weights, model.intercept)))
    logloss = predictions.map(lambda x: computeLogLoss(x[1], x[0])).mean()
    return logloss
...
def parseHashPoint(point, numBuckets):
    ...
    """
    label = point.split(',')[0]
    x = hashFunction(numBuckets, parsePoint(point), True)
    idx = sorted(x.keys())
    val = []
    for key in idx:
        val.append(x[key])
    return  LabeledPoint(label, SparseVector(numBuckets, idx, val))
...
def computeSparsity(data, d, n):
    ...
    """
    sparsity = data.map(lambda x: len(x.features.indices)).mean()/d
    return sparsity
