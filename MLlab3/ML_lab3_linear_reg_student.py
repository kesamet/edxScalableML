...
def parsePoint(line):
    ...
    label = splitLine[0]
    features = splitLine[1:]
    return LabeledPoint(label, features)

parsedSamplePoints = [parsePoint(s) for s in samplePoints]
...
def calcRMSE(labelsAndPreds):
    ...
    s = (labelsAndPreds
         .map(lambda s: squaredError(s[0], s[1]))
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        gradient = (trainData
                    .map(lambda s: gradientSummand(w, s))
                    .reduce(lambda x, y: x + y))
...
def twoWayInteractions(lp):
    ...
    interactions = [a*b for (a,b) in itertools.product(features, features)]
    newFeatures = np.hstack((features, interactions))
